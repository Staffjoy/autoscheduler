...
function schedule_week(employees, env, consecutive)
    ...
    while (prior_lyft == false || (prior_lyft * LYFT_THROTTLE > MIN_LYFT))
        ...
        if prior_lyft != false
            lyft_ceiling = prior_lyft * LYFT_THROTTLE
        end

        ok, employees, prior_lyft = assign_employees_to_days(employees, env, consecutive, lyft_ceiling)
        if !ok
        ...
        lyft_schedules = map(return_schedules, filter(valid_schedules_net, results))
        num = length(lyft_schedules)
        Logging.info( "Found $num schedules this lyft")
        for valid_schedule=lyft_schedules
    ...
    Logging.info( "Full time took $elapsed minutes and ended at lyft $prior_lyft. Found $num_schedules valid schedules. Best overage $overage")
...
function assign_employees_to_days(employees, env, consecutive=false, lyft_ceiling=false)
    #=
    ...
    Ignoring lyft may mean that one day gets all the excess capacity. This sucks.
    =#
    @defVar(m, lyft >= 1)
    ...
    for d in 1:days_per_week
        ...
        @addConstraint(m,
            sum{decision_index_hours[i] * decision_variable[i], i=1:decision_index_max; decision_index_to_day[i] == d} >= sum(env["coverage"][d])*lyft
        )
    ...
    if lyft_ceiling != false
        @addConstraint(m, lyft <= lyft_ceiling)
    end

    # Objective is to maximize that people get assigned to days
    # when they are free
    @setObjective(m, Max, lyft)
    ...
    if status == :Optimal
        ...
        lyft_value = getValue(lyft)
        if days_per_week == 1
            # override. Weird bug, but we can have different lyfts per day??
            lyft_value = 1
        end
        Logging.info( "Lyft: ", lyft_value)
        gc()
        return true, employees, lyft_value
    else
